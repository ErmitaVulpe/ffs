...
impl Db {
        ...
        let db = if db_file_exists {
            ...
            // init tables
            {
                ...
                metadata.insert(
                    ...
                    // this inits at 1 snce 0 is reserved for marker chunks in backends
    ...
    pub fn read(&self) -> Result<ReadTxn> {
        Ok(ReadTxn {
            inner: self.get_redb().begin_read()?,
        })
    }

    pub fn write(&self) -> Result<WriteTxn> {
        Ok(WriteTxn {
            inner: self.get_redb().begin_write()?,
        })
    }
...
impl_both_txn!(ReadTxn);
impl_both_txn!(WriteTxn);
...
impl ReadTxn {
    ...
    pub fn iter_children(
        ...
        let relations = self.inner.open_multimap_table(INODE_RELATION_CHILDREN)?;
        let inodes = self.inner.open_table(INODES)?;
...
// Read-only methods on write transaction that differ in implementaion from ReadTransaction
impl WriteTxn {
    /// Returns a Vec of children of an inode
    pub fn iter_children(&self, id: InodeId) -> Result<Vec<(u64, InodeMeta)>> {
        let relations = self.inner.open_multimap_table(INODE_RELATION_CHILDREN)?;
        let inodes = self.inner.open_table(INODES)?;
        ...
        relations
            ...
            .collect::<Result<Vec<(u64, InodeMeta)>>>()
    }
    ...
    pub fn inode_lookup(&self, path: &InodePath) -> Result<Option<InodeId>> {
        ...
        'seg: for seg in &path.segments {
            for (id, meta) in self.iter_children(current_inode)? {
                if &meta.name == seg {
                    current_inode = id;
                    continue 'seg;
                }
    ...
    #[allow(unused)]
    pub fn list_backends(&self) -> Result<Vec<(BackendId, BackendMeta)>> {
        self.inner
            .open_table(BACKENDS)?
            // .range has to be used here because it returns `Range<'static, K, V>`, and .iter returns
            // `Range<'_, K, V>`
            ...
            .collect::<Result<Vec<(BackendId, BackendMeta)>>>()
    }
}
...
impl WriteTxn {
    ...
    pub fn create_inode(&self, parent: InodeId, id: InodeId, meta: InodeMeta) -> Result<()> {
        ...
        let mut inodes = self.inner.open_table(INODES)?;
        ...
        let mut children_rel = self.inner.open_multimap_table(INODE_RELATION_CHILDREN)?;
        let res = children_rel.insert(parent, id)?;
        debug_assert!(!res);
        let mut parent_rel = self.inner.open_table(INODE_RELATION_PARENT)?;
    ...
    pub fn remove_inode(&self, inode: InodeId) -> Result<()> {
        ...
        match meta.inode_flags.contains(InodeFlags::IS_FILE) {
            true => {
                if meta.size != 0 {
                    let mut chunks_of_inodes = self.inner.open_multimap_table(CHUNKS_OF_INODES)?;
                    let mut chunks_to_drop = self.inner.open_table(CHUNKS_TO_DROP)?;
            ...
            false => {
                let children = self.inner.open_multimap_table(INODE_RELATION_CHILDREN)?;
        ...
        let parent_id = self
            ...
            .value();
        let mut relation_children = self.inner.open_multimap_table(INODE_RELATION_CHILDREN)?;
    ...
    pub fn new_backend_id(&self) -> Result<BackendId> {
        let new_id = {
            let mut meta_table = self.inner.open_table(METADATA)?;
    ...
    pub fn add_backend(&self, id: BackendId, meta: BackendMeta) -> Result<()> {
        let mut backends_table = self.inner.open_table(BACKENDS)?;
    ...
    pub fn reserve_chunk_ids(&self, count: u64) -> Result<RangeInclusive<ChunkId>> {
        let range = {
            let mut table = self.inner.open_table(METADATA)?;
    ...
    pub fn add_temp_chunks<'a>(
        ...
        let mut chunks_table = self.inner.open_table(CHUNKS)?;
        let mut temp_chunks_table = self.inner.open_table(TEMP_CHUNKS)?;
    ...
    pub fn cancel_temp_chunks(&self, ids: impl RangeBounds<u64>) -> Result<bool> {
        let is_empty = {
            let mut temp_chunks = self.inner.open_table(TEMP_CHUNKS)?;
            let mut chunks_to_drop = self.inner.open_table(CHUNKS_TO_DROP)?;
    ...
    pub fn commit_temp_chunks(&self, ids: impl RangeBounds<u64>) -> Result<()> {
        let chunks_table = self.inner.open_table(CHUNKS)?;
        let mut temp_chunks_table = self.inner.open_table(TEMP_CHUNKS)?;
        ...
        let mut backends = self.inner.open_table(BACKENDS)?;
